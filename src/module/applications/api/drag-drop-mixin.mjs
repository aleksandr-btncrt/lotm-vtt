import DragDropLotm from "../../drag-drop.mjs";
import areKeysPressed from "../../utils/areKeysPressed.mjs";

export default function DragDropApplicationMixin(Base) {
  return class DragDropApplication extends Base {
    /** @override */
    _onDragOver(event) {
      const data = DragDropLotm.getPayload(event);
      DragDropLotm.dropEffect = event.dataTransfer.dropEffect = (foundry.utils.getType(data) === "Object")
    ...
    _dropBehavior(event, data) {
      data ??= DragDropLotm.getPayload(event);
      const allowed = this._allowedDropBehaviors(event, data);
      let behavior = DragDropLotm.dropEffect ?? event.dataTransfer?.dropEffect;
